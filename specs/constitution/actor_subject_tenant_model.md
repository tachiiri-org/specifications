...
- ID の一意性スコープ（tenant-scoped vs global例外）
...
> 予約値の詳細と安全柵は `constitution/global_defaults.md` を正とする。
...
> 注: actor 種別の表現方法（claim 名などの具体）は L1 (tool-spec repos) で固定する。
...
> 将来拡張は `constitution/subject_types.md` を起点に行う。
...
### ID Uniqueness Scope (Normative Clarification)

- `actor_id` / `subject_id` は **tenant_id と組で解釈される**（tenant-scoped identifier）。
- `tenant_id` を欠いた状態で actor/subject を参照・キャッシュ・照合してはならない（must-not）。
- global resource（`tenant_scoped=false`）においても、観測上の `tenant_id="__global__"` を用いて **同じ解釈規則**に従う（例外は `global_defaults.md` の安全柵に従う）。
...
> 例外（global resource）を導入する場合の安全柵は `constitution/global_defaults.md` を正とする。
...
- browser 由来の `authorization` は reject される（`constitution/identity.md` を正とする）。
...
- 将来導入は `constitution/delegation_impersonation.md` を起点とし、domain + contract-version で行う。

### 6) Async / Job / Event Context Consistency

- async job / event は必ず `tenant_id` を持つ（global resource の例外は `constitution/global_defaults.md`）。
- async job / event における executor（`actor_id`）は **service/ops actor でなければならない**（must）。
  - human actor を async executor として扱ってはならない（must-not）。
...
> 目的: worker 実行（service/ops actor）と、起点 human（initiator）を混同しない。
...
- tenant_id を欠いた actor/subject のキャッシュ・照合が行われ、越境・誤認可が起きる。
...
- operational_semantics/org_model.md
