...
- **Operational Semantics (operational_semantics/)**
  - 運用上の意味論（idempotency, deletion, persistence, tenancy safety 等）。

- **Interaction Edges (interaction_edges/)**
  - 境界（browser/session/http/webhook 等）と相互作用の意味論。

- **Staging Frames (next_todos/)**
...
- **Machine-contract artifacts (`def/`, `schemas/`, `lint/`)**
  - L1 (tool-spec repos) における機械契約（schema/lint/contract）を保持する領域。
  - Layer 定義は repo ルートの README を参照する。
...
- `constitution/`, `operational_semantics/`, `interaction_edges/` は
  `next_todos/` を参照してはならない。
- `next_todos/` は定義済み仕様を参照してよい（前提・制約の記述のため）。

### Promotion is explicit (Must)

- Staging Frames の内容を「仕様として採用」する場合、次のいずれかへ明示的に昇格する:
  - `constitution/`（語彙・不変条件・責務境界）
  - `operational_semantics/`（運用意味論）
  - `interaction_edges/`（境界・輸送・相互作用）
- 昇格時には互換性（breaking/compatible）と rollout 方針（dual-accept 等）を明示する。

## Boundary & Machine-contract note (Non-normative but important)

- 本リポジトリは憲法/意味論を中心に置く。
- 実際の機械契約（schema/lint/contract）は L1 (tool-spec repos) で保持する。
- したがって constitution は JSONキー形や required keys を固定しない（語彙と禁止のみを固定する）。

## Change Checklist

- 新しい仕様ファイルを追加したか
  - 置き場がこの規約に合っているか（constitution/operational_semantics/interaction_edges/next_todos）
- staging（next_todos）に置くべきものを constitution/operational_semantics/interaction_edges に置いていないか
- constitution/operational_semantics/interaction_edges が staging を参照していないか
- staging を昇格したか
  - 昇格先（constitution/operational_semantics/interaction_edges）が明示され、参照が差し替えられているか
