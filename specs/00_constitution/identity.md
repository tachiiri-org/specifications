...
- assurance / step-up の境界設計（transport/UI/edge）: `30_interaction_edges/step_up.md` / `30_interaction_edges/session.md`
  - 憲法レイヤの意味論は `00_constitution/authn_assurance.md` を正とする。
- key rotation / issuer governance の境界実装詳細: L1
  - 憲法レイヤの原則は `00_constitution/identity_key_rotation.md` を正とする。
- claims set の受理・dual-accept・境界実装詳細: L1
  - 憲法レイヤのガバナンスは `00_constitution/claims_compatibility.md` を正とする。
- 非JWT方式や追加authn手段（境界）: `30_interaction_edges/authn_methods.md`（必要なら10で拡張）
