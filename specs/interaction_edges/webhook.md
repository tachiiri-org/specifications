...
- 返す status / error shape は境界の機械契約（L1 (tool-spec repos)）に従う（固定値を本仕様で持たない）。
...
- 重複は “同一結果の再生” として扱い、副作用を二重実行しない（operational_semantics/idempotency.md）。
...
- 署名検証・リプレイ防止の最低要件は L1 (tool-spec repos) を正とする。
