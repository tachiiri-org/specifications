...
- state-changing は operation の classification により判定される（constitution/operation.md）。
...
> 注: operation schema の参照パスは catalog が唯一の正であり推測してはならない（operational_semantics/payload_schema.md, L1 (tool-spec repos)）。

### Error Semantics (Must)

- adapter の内部エラー詳細を、BFF まで透過させない。
- gateway / BFF は「境界エラー」に正規化する（preserve list 以外は安全側）。
- status / error shape / mapping algorithm の一次情報は L1 (tool-spec repos) を正とする。
...
- 未指定の場合の挙動（例）:
  ...
  - error shape は L1 (tool-spec repos) に従う
...
- boundary JSON は「必須 여부」だけでなく「受理する version 範囲」を明示できなければならない:
  - accepted:
    - explicit_list（例: ["1"]）
    - range（例: min/max）
- payload/schema の互換性（breaking/compatible）は operational_semantics (L0) と L1 (tool-spec repos) を正とする（本ファイルで再定義しない）:
  - operational_semantics/release_compatibility_governance.md
  - operational_semantics/payload_schema.md
...
- contract-version を変更したか
  - accepted 範囲と両対応期間、ロールアウト順序は operational_semantics (L0) と L1 (tool-spec repos) と整合するか
