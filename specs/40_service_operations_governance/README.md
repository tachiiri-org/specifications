# Service Operations Governance (L40)

## Purpose

本ディレクトリは、大規模SaaSを **人間が運用する現実** と
既存の仕様レイヤ（00/20/30）の間を接続する
**運用ガバナンス層（L40）** を定義する。
...
- **00_constitution**
  - 不変の原理・責務・禁止事項を定義する
  - L40 はこれを前提とし、例外を新たに作らない

- **20_operational_semantics**
  - 技術的意味論（idempotency / audit / data / async 等）
  - L40 はそれらを「いつ・誰が・なぜ使うか」を定義する

- **30_interaction_edges**
  - 境界・輸送・確立のルール
  - L40 は transport や header を直接扱わない
