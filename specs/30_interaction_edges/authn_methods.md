...
- “既存トークンの claims を後付けで増やす”変更は breaking とみなす（00_constitution/claims_compatibility.md を正とする）。
...
- issuer allowlist / JWKS rotation / verification cache / failure handling は 30_interaction_edges/identity_key_rotation.md を一次参照とする。
...
- device を主体（subject_type の拡張等）として扱う場合は、別 contract-version が必要（00_constitution/subject_types.md を正とする）。
...
- 30_interaction_edges/identity_key_rotation.md
