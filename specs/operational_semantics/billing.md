...
- audit event は L1 (tool-spec repos) に従う。
...
- 分類と保持は `operational_semantics/data_classification.md` と `operational_semantics/data_persistence.md` に従う。
...
- secret（鍵・APIキー等）の参照方法は `constitution/secrets_and_keys.md` に従う（値は仕様リポジトリへ書かない）。

## Machine-enforced contracts (Moved out of this file)

- operation catalog / boundary / schema の具体キーや形（machine-checkable）は L1 (tool-spec repos) に置く。
