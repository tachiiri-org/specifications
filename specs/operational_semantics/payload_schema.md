...
### Catalog is authoritative for schema paths (Must)

- schema の参照パスは **catalog（L1 (tool-spec repos) の schema.path 等）が唯一の正**である。
- 実装・lint が operation_key から schema パスを推測してはならない（探索禁止）。
- schema の配置は repo_layout の方針に従うが、最終的な正は catalog 参照である。

### Contract version coupling (Must)
...
### Strictness and unknown fields (Must)

- request:
  - unknown field の扱いは operation ごとに明示される（deny 推奨）。
- response:
  - response の unknown field の扱いは **契約として明示**される。
  - 推奨デフォルト:
    - consumer/client は unknown field を無視できる（forward-compatible）。
  - 例外として “strict decode 前提で unknown field を許容しない” 場合は operation（または contract-version）単位で明示する。

> 注:
>
> - “response の unknown top-level field 追加を原則禁止” のような強い制約を採用する場合は、
>   それ自体が互換性戦略（contract-version）と結びつくため、明示が必須。

### Error payload independence (Must)

- error shape は L1 (tool-spec repos) に従い、operation schema の対象外とする。
- business response schema と error schema を混在させない。

## Schema Format (Normative intent)

- format は JSON Schema を使用する（draft 等の固定は L1 (tool-spec repos) 側で定義する）。
- schema が表現すべき最小要素（operation_key, request/response, versioning metadata 等）は L1 (tool-spec repos) の機械契約に従う。

## Machine-enforced contracts (Defined elsewhere)

- JSON Schema の draft、ファイル構造、必須フィールド、互換性判定ロジックは L1 (tool-spec repos) で定義する。
- 本ファイルは「catalogが正」「互換性の意味論」「unknown扱いの明示義務」を正とする。
...
- response の unknown 扱いが不明で、拡張で互換性事故が起きる。

## Related Specifications

- interaction_edges/http.md
- constitution/operation.md
- constitution/repo_layout.md
- operational_semantics/release_compatibility_governance.md
