...
このリポジトリ（以下「Constitution」）は、複数のSaaS/ツールを大規模に運用するための **共通の“憲法”** を定義する。
...
Constitution は **具体の値（timeouts, retry回数, retention日数等）** や **ツール固有の定義（operation catalog / topology / connectors 等）** を持たない。
...
- JSON / schema / lint 実装を提供すること（将来必要なら別リポジトリやテンプレで扱う）
