...
- classification は集合（set/array）として扱われ、複数分類を持ちうる（`00_constitution/operation.md`）。
- `mutate` / `irreversible` / `external_effect` を含む operation は:
  - idempotency を必須とする（運用意味論側の責務）
  - retry は explicit で無制限禁止（運用意味論側の責務）
- `irreversible` / `external_effect` は audit を必須とする（`00_constitution/observability.md`）。
...
- 内部観測（logs/metrics/traces/audit）には decision trace を記録してよい（`00_constitution/policy_decision_trace.md`）。

### 7) SLO & audit retention baseline (Value-free)
...
- claims を AuthZ 入力に追加する変更は breaking とみなす（`00_constitution/authorization.md`）。
