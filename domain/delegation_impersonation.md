...
- 実行主体。AuthZ 入力として扱う唯一の主体（domain/actor_subject_tenant_model.md）。
...
- “既存 token に claims を足して混ぜるだけ”は禁止（domain/authz_and_ops_scaling.md）。
...
- 組織階層との結合（domain/org_model.md の責務）。
