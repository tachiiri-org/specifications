...
- adapter が参照する場合は connector として扱える（domain/dependency_connector_change_semantics.md）。
...
- tenant を跨ぐ “共有グループ” は禁止（導入する場合は別 contract-version で domain 化）。

### 2) Do not pollute AuthZ input claims

- internal boundaries の AuthZ 入力は domain/authorization.md の allowed claims に限定する。
...
- PII を directory からログに落とさない（hash/drop）。

## Recommended integration patterns (Informative)

### Pattern A: Roles/Scopes issuance

- IdP/Directory で group/team membership を解決し、JWT の roles/scopes として発行する。
- adapter PDP は roles/scopes のみを扱い、directory 呼び出しを不要にできる。

### Pattern B: Adapter-side lookup

- adapter 内で directory を参照し、policy evaluation の内部入力として使う。
- connector failure 時の挙動（fail-fast/retry/degraded）を固定する。

## Non-goals

- RBAC/ABAC の完全モデル定義。
- 組織データの同期方式（SCIM等）の仕様化。
...
- domain/dependency_connector_change_semantics.md
