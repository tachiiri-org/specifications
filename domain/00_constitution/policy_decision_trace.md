...
- 利用者へのレスポンスに rule_id / 条件 / 属性値 等の詳細を返さない（domain/00_constitution/authorization.md）。
...
- 準識別子（idempotency-key 等）は hash/drop 方針を適用する（domain/00_constitution/observability.md）。

### 5) Versioning

- decision trace の shape を拡張する場合、互換性を意識する。
- “外部へ explanation を出す”機能を導入する場合は operation 単位で明示し、contract-version を上げる。
