...
- `mutate` / `irreversible` / `external_effect` を含む operation は:
  - idempotency 必須（domain/20_operational_semantics/idempotency.md）
  - retry は explicit で無制限禁止（domain/20_operational_semantics/async_jobs_events.md / rules/async_event_contract.md）
- `irreversible` / `external_effect` は audit 必須（domain/00_constitution/observability.md, rules/audit_log_contract.md）。

### 5) Global resources baseline

- global resource は **例外**であり、暗黙に作らない（domain/20_operational_semantics/data_tenant_safety.md）。
...
- 内部観測（logs/metrics/traces/audit）には decision trace を記録してよい（domain/00_constitution/policy_decision_trace.md）。
...
- claims を AuthZ 入力に追加する変更は breaking とみなす（domain/00_constitution/authorization.md）。
