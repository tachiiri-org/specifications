...
- header/query/body による identity 注入は禁止（domain/headers.md, domain/identity.md）。
...
- “既存トークンの claims を後付けで増やす”変更は breaking とみなす（domain/authorization.md）。
...
- secrets の置き場（domain/secrets_and_keys.md を正とする）。
